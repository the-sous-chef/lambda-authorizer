...
function generatePolicy(principalId: string, effect: string, resource: string, context?: unknown): AWS.Policy {
...
export const authorize: Handler<CustomAuthorizerEvent, APIGatewayProxyResult> = async (
    ...
    try {
        ...
        const decoded = jwt.verify(tokenValue, AUTH0_CLIENT_PUBLIC_KEY, options) as { sub: string };

        return {
            statusCode: 200,
            body: JSON.stringify(generatePolicy(decoded.sub, 'Allow', event.methodArn)),
        };
    } catch (e) {
